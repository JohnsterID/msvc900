...
void CFileDialog::UpdateOFNFromShellDialog()
{
	...
	if (m_bVistaStyle == TRUE)
	{
		...
		m_ofn.nFileOffset = static_cast<WORD>(GetPathName().GetLength() -  GetFileName().GetLength());
		m_ofn.nFileExtension = static_cast<WORD>(GetPathName().GetLength() -  GetFileExt().GetLength());
	}
}
